...
```{r}
#animate_small_trains <- small_trains %>% 
...
#animate_garden_harvest <- garden_harvest %>% 
  ...
  #mutate(day_of_week = wday(date, label = TRUE),
         #cumulative_daily_harvest=cumsum(daily_harvest),
         #mutate(variety = fct_reorder(variety, cumulative_daily_harvest, sum, .desc=TRUE))) %>% 
#ggplot(aes(x=cumulative_daily_harvest, y=day_of_week, fill=variety,
           #alpha=.5))+
   #geom_area() +
